...
#include "featuresGenericTemporalEnergy.h"


vector<string>      FeaturesGenericTemporalEnergy::depandancy()
{
...
void                FeaturesGenericTemporalEnergy::analysis()
{
    
}

variant FeaturesGenericTemporalEnergy::addition(PopulationPtr population)
{
    int nbTime, i, j;
    try {
        nbTime = sSession->getTarget()->getFeature(m_name)->size();
    } catch (const char* e) {
        throw e;
    }
...
matrixPtr FeaturesGenericTemporalEnergy::compare(features& soundsetFeatures, sqlStruct& targetFeatures)
{
    int     i, j;
...
void FeaturesGenericTemporalEnergy::transpose(IndividualPtr individual)
{
    
}

matrixPtr FeaturesGenericTemporalEnergy::neutral()
{
