//
//  computePrevMinima.cpp
//  Orchids
//
//  Created by Antoine Bouchereau on 22/09/14.
//  Copyright (c) 2014 Antoine Bouchereau. All rights reserved.
//

#include "computePrevMinima.h"


int computePrevMinima(vector<float>& a, int current)
{
    int             i, p, n = 2;
    vector<float>   aNext;
    for (i = current; i >= 0; i--)
        aNext.push_back(a[i]);
    float           nextMinVal = aNext[0];
    int             nextMin = 0;
    //getMin(&aNext[0], (int)aNext.size(), nextMinVal, nextMin);
    for (i = 0; i < aNext.size(); i++)
    {
        if(aNext[i] <= nextMinVal)
        {
            nextMinVal  = aNext[i];
            nextMin     = i;
        }
